...
class CronTask
  ...
  # 每日定时任务
  # 0 0 * * * /bin/bash -l -c 'source ~/.bashrc && source ~/.bash_profile && cd /opt/rails-app/slime_crm_hf && rvm use rails_version && rails runner -e production '\''CronTask.daily_cron'\'' >> /opt/rails-app/slime_crm_hf/log/cron.log 2>&1'
  def self.daily_cron
